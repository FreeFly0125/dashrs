use syn::{Error, Generics, LifetimeParam, Type, Result, spanned::Spanned};
...
pub fn type_contains_lifetime(ty: &Type) -> bool {
    match ty {
        ...
        Type::Never(_) => todo!(),
        Type::Paren(_) => todo!(),
        Type::Path(type_path) => type_path.path.segments.iter().any(|segment| match &segment.arguments {
            syn::PathArguments::None => false,
            syn::PathArguments::AngleBracketed(generic_type_parameters) => generic_type_parameters.args.iter().any(|generic| match generic {
        ...
        }) || type_path.qself.as_ref().map(|qself| type_contains_lifetime(&qself.ty)).unwrap_or(false),
        Type::Ptr(_) => todo!(),
