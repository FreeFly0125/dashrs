...
/// Struct modelling the partial user data returned by the `searchGJUsers` endpoint.
///
/// Note that no field `diamonds` exists here. This is consistent with Geometry Dash's behavior, as the GD server exhibit a bug where they do not provide diamonds information, although the client has the UI for it.
